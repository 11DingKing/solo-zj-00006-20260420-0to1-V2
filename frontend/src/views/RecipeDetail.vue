      ...
      <template #header>
        <div class="card-header">
          ...
          <div class="header-actions">
            <el-button type="success" @click="addToShoppingList">
        ...
        <el-col :span="12">
          <div class="recipe-cover">
            <img
              :src="
                recipe.cover_url ||
                'https://picsum.photos/600/400?random=' + recipe.id
              "
              :alt="recipe.name"
            />
          </div>
...
import { recipeApi, weeklyMenuApi, shoppingListApi } from "@/api";
...
const addToShoppingList = async () => {
  ...
  try {
    const res = await shoppingListApi.addFromRecipe(recipe.value.id);
    const { added_count, merged_count } = res.data;
    if (merged_count > 0) {
      ElMessage.success(
        `已加入购物清单（新增 ${added_count} 项，合并 ${merged_count} 项）`,
      );
    } else {
      ElMessage.success("已加入购物清单");
    }
  } catch (error: any) {
    console.error("加入购物清单失败", error);
